...
io.use(function authorization(socket, next) {
  socket.roomName = socket.request.session.roomName;
  Users().first().where({id: socket.request.session.userID})
         .then(function(user){
           socket.user = user;
           next();
         })
...
io.on('connection', function (socket) {
  ...
  socket.on('dog', function(msg){
    ...
    msg.image_url = socket.user.image_url;
    Messages().insert({ usersId: socket.user.id,
                        roomId: msg.roomID,
                        message: msg.message})
      .then( function () {
